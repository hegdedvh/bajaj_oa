...
def is_valid_mobile(phone_number):
    # Remove spaces and check for valid prefixes
    if phone_number.startswith("+91"):
        phone_number = phone_number[3:]
    elif phone_number.startswith("91"):
        phone_number = phone_number[2:]

    # Check if remaining number is 10 digits and within valid range
...
# Question 1: Missing values percentage
...
# Question 2: Female percentage after imputation
gender_counts = Counter(record.get("patientDetails", {}).get("gender", "") for record in data)
mode_gender = gender_counts.most_common(1)[0][0]  # Mode
...
# Question 3: Count of Adults in ageGroup
...
# Question 6: Active/Inactive medicines percentage
active_count = 0
inactive_count = 0
for record in data:
    for med in record.get("consultationData", {}).get("medicines", []):
...
# Question 10: Adding isValidMobile column and counting valid numbers
...
print("valid_mobile_count", valid_mobile_count)


from scipy.stats import pearsonr

# Step 1: Extract age and number of medicines
ages = []
medicine_counts = []
import numpy as np
# Extract ages and medicine counts
for record in data:
    birth_date = record.get("patientDetails", {}).get("birthDate")
    age = calculate_age(birth_date)
    medicines = record.get("consultationData", {}).get("medicines", [])
